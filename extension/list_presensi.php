...
<h3>Daftar Hadir Ujian TA - Tahap 2 - Dibuat 29 Juni 2020</h3>
<table>
    <tr>
        <th>No.</th>
        <th>Ruang</th>
...
$dosencols = get_data('select a.a, c.id, c.nomor_ponsel from (
    SELECT Moderator as a FROM db_tugasakhir.v2_jadwal_ujian
     union  select Penguji1 as a from db_tugasakhir.v2_jadwal_ujian
     union select Penguji2 as a from db_tugasakhir.v2_jadwal_ujian) a
    join db_tugasakhir.v2_jadwal_ujian b on a.a = b.Moderator or a.a = b.Penguji1 or a.a = b.Penguji2 
    join db_tugasakhir.v2_dosen c on a.a = c.nama
    GROUP by a.a, c.id');
$no = 1;
while ( $dosenrow = mysqli_fetch_assoc($dosencols) ) {
    $URI = "http://".$_SERVER['HTTP_HOST']."/ta-tahap2/ujian.php?id=".$dosenrow['id']."&k=".encrypt($dosenrow['id']);
