...
PHREDOC;
    ...
    public static function arrayMultipleNamedInputFormTo2DArray($data)
    {
        ...
        {
            if($value == null)
                continue; // Lewati jika salah satu key di arraynya ada yang null. Kasusnya ini biasanya untuk yang input TYPE_NONE
    ...
    public static function arrayAssocRemoveSomeKeys(array $arrAssoc, array $keys)
    {
        $new = array();
    ...
    public static function arrayAssocRemoveAllByKeyExcept($keptKey, array &$assocArray)
    {
        foreach ($assocArray as $key => $value)
        {
            //pre_print("$key vs $keptKey");
        ...
        //pre_print($assocArray);
    }

    public static function strReplaceNonAlphaNumeric($string, $replacement = '_', $lowercaseOnly = false)
    {
        if($lowercaseOnly)
    ...
    public static function arrayTableAllValuesAreTheSameForColums(array $checkedColumns, array $tableArray)
    {
        if(count($tableArray) < 2)
            return true;

        $firstArray = $tableArray[0];

        for($i = 1; $i < count($tableArray); $i++)
        {
            foreach ($checkedColumns as $checkedColumn)
            {
                if($tableArray[$i][$checkedColumn] != $firstArray[$checkedColumn])
                {
                    return false;
                }
            }
