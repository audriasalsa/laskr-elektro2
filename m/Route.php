...
class Route
{
    ...
    /**
     * @return Current route's path without any URL parameters.
     */
    public function getPathWithoutParams()
    {
    ...
    /**
     * Finds raw path of certain method in a controller
     * @param $controllerName string fill this param using ClassName::class
     * @param $methodName string the name of the method in the controller
     * @return mixed null if the path is not found, the path otherwise
     */
    public function findPathOf($controllerName, $methodName)
    {
        // controller\mahasiswa\PengajuanPembimbingController
        $search = str_replace('controller\\', '', $controllerName);
        $search = rtrim($search, 'Controller');
         ...
         * )
         */
        foreach ($this->_routeMetadata as $rawRoute)
        {
            if($rawRoute[1] == $search)
            {
                if($rawRoute[2] == $methodName)
                {
                    return $rawRoute[0];
                }
            }
    ...
    public function construct()
    {
        ...
        // Masukkan ke variabel path without param secara default
        $this->_pathWithoutParams = $this->_path;
