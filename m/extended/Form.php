...
PHP_HEREDOC;
    ...
    public function addActionButton($id, $value, $onClickAction)
    {
        $button = Input::createActionButton($id, $value, $onClickAction);

        $this->_actionButtons[] = $button;
    }

    public function addActionButtonInput(Input $button)
    {
        $this->_actionButtons[] = $button;
    }

    public function renderActionButtons($extras = '')
    {
        $html = '';

        foreach ($this->_actionButtons as $actionButton)
        {
            $html .= $actionButton->renderControl($extras);
        }
    ...
    {
        for ($i = 0; $i < count($this->_actionButtons); $i++)
        {
            if($this->_actionButtons[$i]->getId() == $id)
                return $this->_actionButtons[$i];
        }
