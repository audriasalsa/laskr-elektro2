...
class UploadedFile
{
    ...
    public function __construct($inputName, UploadedFileModel $model = null)
    {
        ...
        $this->_name    = self::_cleanFileName($file['name'], '_');
    ...
    private static function _cleanFileName($name, $replacement = '_')
    {
        // Allow only: Spaces, ., -
        $search = array('~', '`', '!', '@', '#', '$', '%', '^', '&', '*', '(', ')', '+', '=', '{', '}', '|', '[', ']', '\\', ':', '"', ';', "'", '<', '>', '?', '/');

        return str_replace($search, $replacement, $name);
    }
    ...
    private function _moveUploadedFile($newFileName)
    {
        ...
        // Hint for M-framework devs: Whenever 'Cannot store one or more file' error encountered, comment this line to show the original error message
